<!DOCTYPE html>
 <html lang="es">
    <head>

        <!-- Metas -->
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge" />
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" />
        <meta name="keywords" content="Seventies | Salón & Spá" />
        <meta name="description" content="Ven y relájate y diviértete con nuestro estilo Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."/>
        <meta name="author" content="themezey" />

        <!-- Title  -->
        <title>Seventies</title>
        <!-- Favicon -->
        <link rel="shortcut icon" href="images/logo_black.png" />
        <!-- Fonts -->
        <link href="https://fonts.googleapis.com/css?family=Poppins:200,300,400,500,600,700,800" rel="stylesheet" />
        <link href="https://fonts.googleapis.com/css?family=Raleway:200,300,400,600,700,800" rel="stylesheet">

        <!-- Plugins -->
        <link rel="stylesheet" href="css/plugins.css" />
        <!-- Core Style Css -->
        <link rel="stylesheet" href="css/style.css" />
        <link rel="stylesheet" href="css/custom.css" />

        
    </head>

    <body>

        <!-- =====================================
        ==== Start Loading -->
        <div class="loading">
            <div class="text-center middle">
                <div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div>
            </div>
        </div>
        <!-- End Loading ====
        ======================================= -->

        <!-- =====================================
        ==== Start Navbar -->
        <nav class="navbar navbar-expand-lg">
            <div class="container">
              <a class="logo" href="slider.html#">
                <img src="images/seventie-chip-negro.svg" alt="logo">
              </a>
              <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                <span class="icon-bar"><i class="fas fa-bars"></i></span>
              </button>

              <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ml-auto">
                  <li class="nav-item">
                    <a class="nav-link active" href="slider.html#" data-scroll-nav="0">Home</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="slider.html#" data-scroll-nav="1">Nosotros</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="slider.html#" data-scroll-nav="2">Servicios</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="slider.html#" data-scroll-nav="3">Galeria</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="#contact" data-scroll-nav="4">Contacto</a>
                  </li>
                </ul>
              </div>
            </div>
        </nav>
        <!-- End Navbar ====
        ======================================= -->

        <!-- =====================================
        ==== Start Header -->
        <header class="header slider-fade" data-scroll-index="0">
            <div class="container-fluid">
                <div class="row">
                    <div class="owl-carousel owl-theme full-width">
                        <div class="text-center item bg-img" data-overlay-dark="7" data-background="images/slide/3.jpg" style="background-repeat: no-repeat;background-position: center top;">
                            <div class="v-middle caption">
                                <h5>Sintoniza <span> tu Sueños</span></h5>
                                <h1 class="bold sizeletra">Realzamos tu Belleza</h1>
                                
                               <a href="#contact" class="buton">Contáctanos</a>
                            </div>
                        </div>
                        <div class="text-center item bg-img" data-overlay-dark="7" data-background="images/slide/4.jpg" style="background-repeat: no-repeat;background-position: center top;">
                            <div class="v-middle caption">
                                <h5>Sintoniza <span> tu Sueños</span></h5>
                                <h1 class="bold sizeletra">Realzamos tu Belleza</h1>
                                
                               <a href="#contact" class="buton">Contáctanos</a>
                            </div>
                        </div>
                        <div class="text-center item bg-img" data-overlay-dark="7" data-background="images/slide/9.jpg" style="background-repeat: no-repeat;background-position: center top;">
                            <div class="v-middle caption">
                                <h5>Sintoniza <span> tu Sueños</span></h5>
                                <h1 class="bold sizeletra">Realzamos tu Belleza</h1>
                                
                               <a href="#contact" class="buton">Contáctanos</a>
                            </div>
                        </div>
                    
                    </div>
                </div>
            </div>

            <div class="arrow">
                <a href="slider.html#" data-scroll-nav="1">
                    <i class="fas fa-chevron-down"></i>
                </a>
            </div>
        </header>
        <!-- End Header ====
        ======================================= -->

        <!-- =====================================
        ==== Start Header -->
        <section class="hero" data-scroll-index="1">
            <div class="intro bg-gray1 text-center">
                <div class="container">
                    <div class="row">
                        <div class="offset-lg-2 col-lg-8 content wow fadeInUp" style="visibility: visible; animation-name: fadeInUp;">
                            <a class="logo-img" href="#">
                                <img src="images/seventie-negro1.svg" alt="logo">
                            </a>
                            <div class="section-head">
                                <h4>Salón <span>&</span> Spa</h4>
                                <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.</p>
                            </div>
                            <img src="images/signature.png" class="firma">
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- =====================================
        ==== Start Header -->

       <!-- =====================================
        ==== Start services -->
        <section class=" section-padding text-center bgsev" data-scroll-index="2">
            <div class="container" >
                <div class="row">
                    <div class="offset-md-2 col-md-8">
                        <div class="section-head">
                            <h4 class="white">Nuestros <span class="white">Servicios</span></h4>
                            <p class="white">Lorem Ipsum is simply dummy text of the printing and type setting industry. Lorem Ipsum has been the industry's standard dummy text ever since</p>
                        </div>
                    </div>
                    <div class="clear-fix"></div>
                    <div class="col-lg-4 col-md-6">
                        <div class="item wow fadeInUp">
                            <div class="img-contenedor">
                                <img src="images/maquillaje.jpg">
                            </div>    
                            <br><br>
                            <h6 class="white">MAQUILLAJE</h6>
                            <br>
                            <ul class="white">
                                <li>Novias</li>
                                <li>Asesoría en Imagen</li>
                                <li>Spa</li>
                                <li>Barber shop</li>
                                <li>Manicura</li>
                                <li>Corte cabellos</li>
                            </ul>
                            <!-- <p class="white">Servicios : Maquillaje (Novias, Asesoría en Imagen. Spa, Barber shop, Manicura, Corte cabellos(general)</p> -->
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6 bord">
                       <div class="item wow fadeInUp">
                            <div class="img-contenedor">
                                <img src="images/coffee.jpg">
                            </div> 
                            <br><br>
                            <h6 class="white">COFFEE BAR</h6>
                            <br>
                            <ul class="white">
                                <li>Whisky (2 tipo)</li>
                                <li>Ron (2 tipo)</li>
                                <li>Cervezas artesanal draft</li>
                                <li>Vinos (blanco, rosado, tinto)</li>
                                <li>Postres y Dulce (por niños)</li>
                            </ul>
                            <!-- <p class="white">Whisky (2 tipo), Ron (2 tipo) , cervezas artesanal draft, Vinos (blanco, rosado, tinto) - Postres y Dulce (por niños)</p> -->
                        </div>
                    </div>
                    <div class="col-lg-4 col-md-6">
                       <div class="item wow fadeInUp">
                            <div class="img-contenedor">
                                <img src="images/stuff.jpg">
                            </div> 
                            <br><br>
                            <h6 class="white">SOUVENIRS</h6>
                            <br>
                            <ul class="white">
                                <li>Ventas de productos comercial</li>
                                <li>Artesanal mujeres/ hombres.</li>
                                <li>Ambiente de lectura</li>
                                <li>Juegos de mesa</li>
                                <li>Música y video.</li>
                            </ul>
                            <!-- <p class="white">Ventas de productos comercial, artesanal mujeres/ hombres. Ambiente de lectura, juegos de mesa, música y video.</p> -->
                        </div>
                    </div>
                    <div class="clearfix"></div>
                </div>
            </div>
        </section>
        <!-- End services ====
        ======================================= -->

        <!-- =====================================
        ==== Start Portfolio -->
        <section class="portfolio section-padding pb-0" data-scroll-index="3">
            <div class="container-fluid">

                <div class="row">

                    <div class="offset-md-2 col-md-8">
                        <div class="section-head">
                            <h4>Descubre <span>nuestro </span> Estilo</h4>
                            <p>Lorem Ipsum is simply dummy text of the printing and type setting industry. Lorem Ipsum has been the industry's standard dummy text ever since</p>
                        </div>
                    </div>
                    <div class="clear-fix"></div>

                    <div class="container">
 
                        <div class="filtering mb-50">
                            <span data-filter='*' class="active">Todos</span>
                            <span data-filter='.hombres'>Hombres</span>
                            <span data-filter='.Mujeres'>Mujeres</span>
                            <span data-filter='.ninos'>Niños</span>
                        </div>
                    </div>

                    <div class="gallery text-center full-width">    

                        <div class="col-md-4 o-hidden items ninos">
                            <div class="item-img wow slideInRight">
                                <img src="images/galeria/1.jpg" alt="image">
                                <div class="item-img-overlay valign">
                                    <div class="overlay-info full-width vertical-center">
                                        <h6>Loren Ipsum</h6>
                                        <p>Loren Ipsum Dolot amet</p>
                                    </div>
                                    <a href="images/galeria/1.jpg" class="popimg">
                                        <i class="fas fa-image"></i>
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4 o-hidden items Mujeres">
                            <div class="item-img wow slideInUp">
                                <img src="images/galeria/2.jpg" alt="image">
                                <div class="item-img-overlay valign">
                                    <div class="overlay-info full-width vertical-center">
                                        <h6>Loren Ipsum</h6>
                                        <p>Loren Ipsum Dolot amet</p>
                                    </div>
                                    <a href="images/galeria/2.jpg" data-lity>
                                        <i class="fas fa-file"></i>
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4 o-hidden items hombres">
                            <div class="item-img wow slideInDown">
                                <img src="images/galeria/3.jpg" alt="image">
                                <div class="item-img-overlay valign">
                                    <div class="overlay-info full-width vertical-center">
                                        <h6>Loren Ipsum</h6>
                                        <p>Loren Ipsum Dolot amet</p>
                                    </div>
                                    <a href="images/galeria/3.jpg" class="popimg">
                                        <i class="fas fa-image"></i>
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4 o-hidden items ninos">
                            <div class="item-img wow slideInRight">
                                <img src="images/galeria/4.jpg" alt="image">
                                <div class="item-img-overlay valign">
                                    <div class="overlay-info full-width vertical-center">
                                        <h6>Loren Ipsum</h6>
                                        <p>Loren Ipsum Dolot amet</p>
                                    </div>
                                    <a href="images/galeria/4.jpg" class="popimg">
                                        <i class="fas fa-image"></i>
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4 o-hidden items Mujeres">
                            <div class="item-img wow slideInLeft">
                                <img src="images/galeria/5.jpg" alt="image">
                                <div class="item-img-overlay valign">
                                    <div class="overlay-info full-width vertical-center">
                                        <h6>Loren Ipsum</h6>
                                        <p>Loren Ipsum Dolot amet</p>
                                    </div>
                                    <a href="images/galeria/5.jpg" data-lity>
                                        <i class="fas fa-file"></i>
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="col-md-4 o-hidden items hombres">
                            <div class="item-img wow slideInUp">
                                <img src="images/galeria/6.jpg" alt="image">
                                <div class="item-img-overlay valign">
                                    <div class="overlay-info full-width vertical-center">
                                        <h6>Loren Ipsum</h6>
                                        <p>Loren Ipsum Dolot amet</p>
                                    </div>
                                    <a href="images/galeria/6.jpg" data-lity>
                                        <i class="fas fa-file"></i>
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="clear-fix"></div>

                    </div>
                </div>
            </div>
        </section>
        <!-- End Portfolio ====
        ======================================= -->

        <!-- =====================================
        ==== Start Team -->
        <section class="team section-padding bg-gray" data-scroll-index=".." >
            <div class="container">
                <div class="row">
                    
                    <div class="offset-md-2 col-md-8">
                        <div class="section-head">
                            <h4>Conoce a <span>Nuestros</span> Estilistas</h4>
                            <p>Lorem Ipsum is simply dummy text of the printing and type setting industry. Lorem Ipsum has been the industry's standard dummy text ever since</p>
                        </div>
                    </div>
                    <div class="clear-fix"></div>

                    <div class="owl-carousel owl-theme full-width text-center">
                        <div class="titem wow fadeInUp">
                            <div class="author-img">
                                <img src="images/team/1.jpg" alt="">
                            </div>
                            <div class="info">
                                <h6>Luis bernal</h6>
                                <span>Estilista Profesional</span>
                            </div>
                        </div>
                        <div class="titem wow fadeInUp">
                            <div class="author-img">
                                <img src="images/team/3.jpg" alt="">
                            </div>
                            <div class="info">
                                <h6>Balbino Gamboa</h6>
                                <span>Masajista </span>

                            </div>
                        </div>
                        <div class="titem wow fadeInUp">
                            <div class="author-img">
                                <img src="images/team/2.jpg" alt="">
                            </div>
                            <div class="info">
                                <h6>Raul Marquez</h6>
                                <span>Estilista Profesional</span>
                            </div>
                        </div>
                        <div class="titem wow fadeInUp">
                            <div class="author-img">
                                <img src="images/team/1.jpg" alt="">
                            </div>
                            <div class="info">
                                <h6>Luis Felipe</h6>
                                <span>Barbero</span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- End Team ====
        ======================================= -->


        <!-- =====================================
        ==== Social Fixed -->
        <div class="header-fix-right" style="position: fixed; bottom: 10%; width: 32px;">
            <ul class="header-fix-redes">
                <li >
                    <a href="https://www.facebook.com/" target="_blank" style="">
                        <img src="images/icon/facebook-f.svg" class="img_social">
                    </a>
                </li>
                <li>
                    <a href="https://www.facebook.com/" target="_blank">
                        <img src="images/icon/instagram-f.svg" class="img_social">
                    </a>
                </li>
                <li>
                    <a href="https://www.facebook.com/" target="_blank">
                        <img src="images/icon/youtube-f.svg" class="img_social">
                    </a>
                </li>
            </ul>
            <span class="header-fix-text">Síguenos:</span>
        </div>
        <!-- Social FIxed ====
        ======================================= -->

        <!-- =====================================
        ==== Start Contact -->
        <section class="contact" data-scroll-index="4" >
            <div class="contact-info bg-img" data-overlay-dark="6" data-background="images/8.jpg" style="background-repeat: no-repeat;background-position: center top;">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="item valign">
                                <span class="icon"><i class="fas fa-envelope"></i></span>
                                <div class="cont vertical-center">
                                    <h6>Email</h6>
                                    <p>info@seventies.pe</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div class="item valign">
                                <span class="icon"><i class="fas fa-location-arrow"></i></span>
                                <div class="cont vertical-center">
                                    <h6>Dirección</h6>
                                    <p>Bolivar 268 Miraflores Lima, PERÚ</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <div class="item valign">
                                <span class="icon"><i class="fas fa-phone"></i></span>
                                <div class="cont vertical-center">
                                    <h6>Telefono</h6>
                                    <p>+51 (01) 7654321</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="container section-padding" id="contact">
                    
                <div class="section-head">
                    <h4>Con<span>tác</span>tanos</h4>
                    <p>Lorem Ipsum is simply dummy text of the printing and type setting industry.</p>
                </div>

                <div class='form o-hidden offset-lg-2 col-lg-8 offset-md-1 col-md-10'>
                    <input type='hidden' name='form-name' value='contact-form' />
                    <div class="controls wow fadeInUp">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <input type="text" id="nombre" name="nombre" placeholder="Nombres" required="required" />
                                </div>
                                <div class="form-group">
                                    <input type="email" id="email" name="email" placeholder="Email" required="required" />
                                </div>
                                <div class="form-group">
                                    <input type="text" id="asunto" name="asunto" placeholder="Asunto" required="required" />
                                </div>
                                <div class="form-group">
                                    <textarea id="mensaje" name="mensaje" placeholder="Mensaje" rows="4" required="required"></textarea>
                                </div>
                                <div id="mail-status"></div>
                                <button type="submit" name="submit" class="btn-seven" onClick="sendContact();">Enviar Mensaje</button>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
            <div class="themap">
                <div class="map-toggle"><i class="fas fa-map-marker-alt"></i></div>
                <div id="ieatmaps" class="map"></div>
            </div>
        </section>
        <!-- End Contact ====
        ======================================= -->

        <!-- =====================================
        ==== Start Footer -->
        <footer class="page-footer">
              <div class="container" align="center">
                <div class="row">
                  <div class="col-md-12">
                    <img src="images/seventie-chip-blanco.svg" class="img-responsive abt_img2 ">
                    <p class="grey-text text-lighten-4 text_foot img_foo foo_bt">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since.</p>
                     
                  </div>
                </div>
              </div>

            <div class="container" align="center">
                <div class="row">
                    <div class="col-md-12 img_foo">
                        <a href="https://www.behance.net/ventas326d" target="_blank"><span class="social"> <i class="fab fa-behance"></i></span></a>
                        <a href="https://www.linkedin.com/company/update-global-marketing" target="_blank"><span class="social"> <i class="fab fa-linkedin-in"></i></span></a>
                        <a href="https://www.facebook.com/updatemarketing/" target="_blank"><span class="social"> <i class="fab fa-facebook-f"></i></span></a>
                        <a href="https://www.instagram.com/update.pe/" target="_blank"><span class="social"> <i class="fab fa-instagram"></i></span></a>
                    </div>
                </div>
            </div>

            <div class="footer-copyright bgfoote">
                <div class="container" style="font-size: 10px;">
                    <div class="row" align="center">
                        <div class="col-md-12" >
                             <p class="foo_bt">Update Global Marketing &copy; | info@update.pe </p>   
                        </div>
                    </div>
                </div>
            </div>
        </footer>
        <!-- End Footer ====
        ======================================= -->
        
       
        <!-- jQuery -->
        <script src="js/jquery-3.0.0.min.js"></script>
        <script src="js/jquery-migrate-3.0.0.min.js"></script>

<script>
            function sendContact(){
                var valid;
                valid = validateContact();
                if(valid) {
                    jQuery.ajax({
                        url: "contact_form.php",
                        data:'nombre='+$("#nombre").val()+'&email='+$("#email").val()+'&asunto='+$("#asunto").val()+'&mensaje='+$("#mensaje").val(),
                        type: "POST",
                        success:function(data){
                            $("#mail-status").html(data);
                        },
                        error:function (){}
                    });
                }
            }
            function validateContact(){
                var valid = true;
                if(!$("#nombre").val()) {
                    $("#nombre").css('background-color','#f28282');
                    valid = false;
                }
                if(!$("#email").val()) {
                    $("#email").css('background-color','#f28282');
                    valid = false;
                }
                if(!$("#email").val().match(/^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/)) {
                    $("#email").css('background-color','#f28282');
                    valid = false;
                }
                if(!$("#asunto").val()) {
                    $("#asunto").css('background-color','#f28282');
                    valid = false;
                }
                if(!$("#mensaje").val()) {
                    $("#mensaje").css('background-color','#f28282');
                    valid = false;
                }
                return valid;
            }
        </script>

        <!-- popper.min -->
        <script src="js/popper.min.js"></script>

        <!-- bootstrap -->
        <script src="js/bootstrap.min.js"></script>

        <!-- scrollIt -->
        <script src="js/scrollIt.min.js"></script>

        <!-- jquery.waypoints.min -->
        <script src="js/jquery.waypoints.min.js"></script>

        <!-- jquery.counterup.min -->
        <script src="js/jquery.counterup.min.js"></script>

        <!-- owl carousel -->
        <script src="js/owl.carousel.min.js"></script>

        <!-- jquery.magnific-popup js -->
        <script src="js/jquery.magnific-popup.min.js"></script>

        <!-- lity.min js -->
        <script src="js/lity.min.js"></script>

        <!-- stellar js -->
        <script src="js/jquery.stellar.min.js"></script>

        <!-- isotope.pkgd.min js -->
        <script src="js/isotope.pkgd.min.js"></script>

        <!-- Map -->
        <script src="js/map.js"></script>

        <!-- validator js -->
        <script src="js/validator.js"></script>

        <!-- custom scripts -->
        <script src="js/scripts.js"></script>

        <!-- google map api -->
        <script async defer
            src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBK7lXLHQgaGdP3IvMPi1ej0B9JHUbcqB0&callback=initMap">
        </script>

        <!-- wow js -->
        <script src="js/wow.min.js"></script>

        <script>
            new WOW().init();
        </script>

    </body>
</html>